    ...
    for item in raw_qas:
        ...
        if not q_key:
            if mapping.allow_unknown:
                # keep unknowns out of required list but still include in normalized for transparency
                normalized.append({
                    "key": str(label) or "unknown",
                    "question_text": str(label) or "Unknown",
                    "answer_text": str(ans)
                })
                continue
...
def _xml_minimal(user: Dict[str, Any], qas: List[Dict[str, Any]]) -> str:
    """
    Minimal XML that mirrors what your backend expects based on logs:
    - both snake_case and TitleCase for person fields
    - <QuestionAnswers> with <QA><Question/><Answer/></QA>
    - consolidated text/json mirrors
    """
    req = Element("Request")

    # IDs
    for tag in ("request_id", "RequestId"):
        SubElement(req, tag).text = user.get("request_id", "")
    for tag in ("result_key", "ResultKey"):
        SubElement(req, tag).text = user.get("result_key", "")

    # Person fields (duplicate casings)
    ...
    for tag in ("phone_number", "PhoneNumber"):
        SubElement(req, tag).text = user.get("phone_number", "")

    # Birth date: write common aliases
    bd = user.get("birth_date", "")
    for tag in ("date_of_birth", "DateOfBirth", "dob", "DOB", "date", "Date"):
        SubElement(req, tag).text = bd

    # QA (TitleCase container seen in backend logs)
...
def _xml_superset(user: Dict[str, Any], qas: List[Dict[str, Any]]) -> str:
    """
    Broader XML variant (fallback) – includes multiple container spellings.
    """
    req = Element("Request")
...
def _call_backend(xml_body: str, cid: str) -> Dict[str, Any]:
    ...
    # Some backends return the final object immediately
...
def adapter():
    ...
    # Normalization-only (no backend call)
    ...
    # Build XML and call backend with fallback
    debug_xml = str(request.headers.get("X-Debug-XML", "")).lower() in ("1", "true", "yes")

    xml_min = _xml_minimal(user, qas)
    xml_sup = None
    try:
        backend_result = _call_backend(xml_min, g.cid)
    except Exception as e1:
        logger.warning("Minimal XML failed; retrying with superset. cid=%s err=%s", g.cid, e1)
        xml_sup = _xml_superset(user, qas)
        try:
            backend_result = _call_backend(xml_sup, g.cid)
        except Exception as e2:
            logger.exception("Backend call failed cid=%s (even with superset)", g.cid)
            details = {"first_try": str(e1), "second_try": str(e2)}
            if debug_xml:
                details["xml_minimal"] = xml_min
                details["xml_superset"] = xml_sup
            return _json_error(502, "backend_error", "Upstream backend call failed", details)
    ...
    if debug_xml:
        dbg = {"minimal": xml_min}
        if xml_sup:
            dbg["superset"] = xml_sup
        resp_payload["debug_xml"] = dbg
