...
def _validate(payload: Dict[str, Any], mapping: Mapping) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    user = {
        ...
        "result_key":   payload.get("result_key") or "",   # forwarded as response_id to backend
        "test_mode":    payload.get("test_mode") or "live",
    }

    # allow adapter to auto-generate a result_key if missing (keeps backend unchanged)
    if not user["result_key"]:
        user["result_key"] = f"rk-{uuid.uuid4().hex[:8]}-{int(time.time())}"
